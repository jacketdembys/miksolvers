...
class ResMLPSum(nn.Module):
    def __init__(self, input_dim, hidden_size, output_dim, num_blocks):
        ...
        self.name = "ResMLP [i{}, n{}, b{}, l2, o{}]".format(str(input_dim), str(hidden_size).replace("[","").replace("]",""), str(num_blocks), str(output_dim))
...
class ResMDNMLPSum(nn.Module):
    def __init__(self, input_dim, hidden_size, output_dim, num_blocks, K=5):
        super().__init__()
        self.K = K
        self.D = output_dim

        self.name = f"ResMDNMLP [i{input_dim}, h{hidden_size}, b{num_blocks}, K{K}, o{output_dim}]"

        self.input = nn.Linear(input_dim, hidden_size)

        self.blocks = nn.Sequential()
        for i in range(num_blocks):
            self.blocks.add_module(f'block{i+1}', ResidualBlockSum(hidden_size, hidden_size))

        # MDN heads
        self.pi_fc = nn.Linear(hidden_size, K)                  # mixture logits
        self.mu_fc = nn.Linear(hidden_size, K * output_dim)     # means
        self.logsig_fc = nn.Linear(hidden_size, K * output_dim) # log sigmas

    def forward(self, x):
        h = self.input(x)
        h = self.blocks(h)

        logits_pi = self.pi_fc(h)  # [B, K]
        mu = self.mu_fc(h).view(-1, self.K, self.D)  # [B, K, D]
        log_sigma = self.logsig_fc(h).view(-1, self.K, self.D)  # [B, K, D]

        # Deterministic “single solution” output for compatibility (optional)
        k = torch.argmax(logits_pi, dim=1)  # [B]
        y_pred = mu[torch.arange(mu.size(0), device=mu.device), k, :]  # [B, D]

        mdn_params = (logits_pi, mu, log_sigma)
        return y_pred, mdn_params
...
class MDNMLP(nn.Module):
    def __init__(self, input_dim, h_sizes, output_dim, K=5):
        super().__init__()
        self.name = f"MDNMLP[{input_dim},{h_sizes},{output_dim},K={K}]"
        self.K = K
        self.D = output_dim

        self.input_fc = nn.Linear(input_dim, h_sizes[0])
        self.hidden = nn.ModuleList([
            nn.Linear(h_sizes[i], h_sizes[i+1]) for i in range(len(h_sizes)-1)
        ])

        # heads
        self.pi_fc = nn.Linear(h_sizes[-1], K)              # mixture logits
        self.mu_fc = nn.Linear(h_sizes[-1], K * output_dim) # means
        self.logsig_fc = nn.Linear(h_sizes[-1], K * output_dim) # log sigma
    ...
    def forward(self, x):
        ...
        # deterministic prediction for compatibility:
        # pick the mean of the most probable component
        k = torch.argmax(logits_pi, dim=1)  # [B]
        y_pred = mu[torch.arange(mu.size(0), device=mu.device), k, :]  # [B,D]
